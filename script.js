// script.js - Version ULTIME avec TOUS les effets + ACTIONS/DÉCORS
// CORRIGÉ : Gestion intelligente des types pour actions.js
...
class PromptGenerator {
    constructor() {
        this.userData = {};
        // Mapping de secours pour les types qui n'existent pas dans actions.js
        this.typeMapping = {
            'default': 'pirate',      // fallback élégant
            'fairy': 'fairy',          // à ajouter dans actions.js si besoin
            'vampire': 'vampire',      // à ajouter dans actions.js si besoin
            'elf': 'elf',              // à ajouter dans actions.js si besoin
            'superman': 'superman',    // existe déjà
            'cowgirl': 'cowgirl',      // existe déjà
            'pirate': 'pirate',        // existe déjà
            'princess': 'princess',    // existe déjà
            'siren': 'siren',          // existe déjà
            'cavewoman': 'cavewoman',  // existe déjà
            'athena': 'athena',        // existe déjà
            'avatar': 'avatar',        // existe déjà
            'avatarwarrior': 'avatarwarrior', // existe déjà
            'avatarchief': 'avatarchief'      // existe déjà
        };
    }

    /**
     * Récupère le bon type pour actions.js avec fallback intelligent
     */
    getActionType(personnageKey) {
        const country = countries[personnageKey];
        if (!country) return 'pirate'; // Fallback ultime
        
        let type = country.type || personnageKey;
        
        // Vérifier si ce type existe dans personnageActions
        if (typeof personnageActions !== 'undefined' && personnageActions[type]) {
            return type; // OK, le type existe
        }
    ...
    collectUserData() {
        ...
        // Récupérer les gestes correctement (correction des IDs)
...
- La PARTIE 2 commencera avec le résultat FINAL déjà visible`;
    ...
    generatePart2() {
        ...
        // 👇 OBTENIR LE TYPE POUR ACTIONS.JS
...
ARRIÈRE-PLAN :
${this.userData.background === 'custom' ? this.userData.customBackground : decorAleatoire}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Gestionnaire pour les gestes manquants
    const bisousCheckbox = document.getElementById('gesteBisous');
...
// Vérification que personnageActions existe
