...
class PromptGenerator {
    ...
    // 👇 FONCTION POUR GÉNÉRER LES TEXTES FLOTTANTS
...
- Parfait pour les réseaux sociaux et l'interaction avec le spectateur`;
    ...
    generatePart1() {
        ...
        // 👇 TEXTES FLOTTANTS POUR PARTIE 1
...
14. YEUX 8K NATURELS : Iris détaillés, vaisseaux visibles, reflets nets${alienLine}`;
    ...
    generateFullPrompt() {
        ...
        // 👇 Récupérer le type du personnage pour les actions/décors
        const personnageType = countries[this.userData.country]?.type || this.userData.country;
        
        // 👇 Choisir un décor aléatoire parmi les 3 du personnage
        const decorAleatoire = (typeof getRandomDecor === 'function') ? getRandomDecor(personnageType) : (countries[this.userData.country]?.background || 'studio professionnel');
        
        // 👇 Choisir une action aléatoire parmi les 3 du personnage
        const actionAleatoire = (typeof getRandomAction === 'function') ? getRandomAction(personnageType) : 'elle danse sensuellement';
        
        // Générer les parties
        let part1 = this.generatePart1();
        let part2 = this.generatePart2();
        
        // 👇 AJOUTER LE DÉCOR ET L'ACTION DANS LA PARTIE 2
        // Remplacer l'arrière-plan standard par le décor aléatoire
        part2 = part2.replace(
            /ARRIÈRE-PLAN :[\s\S]*?(?=\n\n|$)/, 
            `ARRIÈRE-PLAN :\n${decorAleatoire}`
        );
        
        // Ajouter l'action spéciale avant les effets
        part2 = part2.replace(
            /(JEUX DE SÉDUCTION ACCRUS :[\s\S]*?)(?=JEUX AVEC LA NOUVELLE TENUE)/,
